clear all
clear
[labels, features] = libsvmread('../Data/w8a.txt');


n = length(labels); % n denotes the number of datapoints in the entire w8a dataset
d = length(features(1, :));
...
fprintf("No of -1(0) and 1(1) in the dataset is %0.4d and %0.4d respectively\n", sum(Y(:) == 0), sum(Y(:) == 1));
N = 2000;
idx = randperm(n,N);
lambda = 1 / N;
X = X(idx,:)';
...
Hw = zeros(d, N);   % maintians the product Hw in a (d, n) array
...
for ep = 1 : epochs
    for rep = 1 : N / (N_subset)
        ...
        if ep > 1
            prev_file_name = "../data_store/w8a/epochs/" + "Epoch_" + string(ep - 1) + "_rep_" + string(rep);
        ...
        file_name = "../data_store/w8a/epochs/" + "Epoch_" + string(ep) + "_rep_" + string(rep);
...
%% Implement Greedy IQN (Memory efficient version, M = 0)
w = w0;
ta = repmat(w, [1 N]); % tracks all z values in a (d, n) array
...
save("../data_store/w8a/all_vars/w8a_all_vars_before_SN")
...
save("../data_store/w8a/all_vars/w8a_all_vars_till_SN")
%% Plots
x_axis = 1:conv_iters;
...
semilogy(1:conv_iters, y_axis_stochastic_newton_eff_space(1:conv_iters), '-squarem', 'LineWidth', 4);
...
save_file_name = "../Matlab_plots/save_load_w8a_p_21_" + "_N_" + string(N) + "_N_subset_" + string(N_subset);
