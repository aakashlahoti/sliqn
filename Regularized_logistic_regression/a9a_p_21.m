...
% load data: variables X, Y, n, d
load ../Data/a9a_new.mat
disp(n); disp(d);
...
for ep = 1 : epochs
    for rep = 1 : N / (N_subset)
        ...
        if ep > 1
            prev_file_name = "../data_store/a9a/epochs/" + "Epoch_" + string(ep - 1) + "_rep_" + string(rep);
        ...
        file_name = "../data_store/a9a/epochs/" + "Epoch_" + string(ep) + "_rep_" + string(rep);
...
%% Implement IGS (no correction term needed)
w = w0;
ta = repmat(w, [1 N]); % tracks all z values in a (d, n) array
...
%% Implement SL-IQN (no correction term needed)
w = w0;
ta = repmat(w, [1 N]); % tracks all z values in a (d, n) array
...
save("../data_store/a9a/all_vars/a9a_all_vars_till_NIM")
...
save("../data_store/a9a/all_vars/a9a_all_vars_till_SN")
%% Plots
x_axis = 1:conv_iters;
...
save_file_name = "../Matlab_plots/save_load_a9a_p_21_" + "_N_" + string(N) + "_N_subset_" + string(N_subset);
